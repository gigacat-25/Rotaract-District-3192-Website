  ...
  return (
    <main className="lg:ml-64 min-h-screen relative overflow-visible pb-24 ">
      ...
      <section className="p-8 md:p-12 space-y-12">
        {/* Filter Bar */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 -container-low/50 backdrop-blur-md p-2 rounded-2xl border border-white/20 shadow-sm">
        ...
        <div className="grid gap-8">
          {jobs.map((job, idx) => (
            <div key={idx} className={`group -container-lowest/70 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/40 shadow-xl hover:shadow-2xl hover:bg-white transition-all duration-500 overflow-hidden relative ${job.status === 'closed' ? 'opacity-50' : ''}`}>
              <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-12">
                <div className="flex-1 space-y-6">
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    {job.status === 'closed' && (
                      <span className="text-[9px] font-bold uppercase tracking-widest px-4 py-1.5 rounded-lg -container-high text-on-surface-variant border border-outline/5">Sequence Terminated</span>
                    )}
                ...
                <div className="shrink-0 flex flex-col items-start xl:items-end gap-6">
                  <a href={job.status === 'active' ? (job.apply_url || '#') : '#'} className={`group/btn relative px-12 py-5 font-bold uppercase tracking-widest text-xs rounded-2xl w-full xl:w-auto text-center transition-all shadow-xl
                    ${job.status === 'active' 
                      ...
                      : '-container-high text-on-surface-variant cursor-not-allowed opacity-50'}`}>
                    {job.status === 'active' ? (
